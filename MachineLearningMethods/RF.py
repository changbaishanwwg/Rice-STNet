...
save_dir = create_result_subfolders("RF")

test_size = 0.25
random_state = 8

model_out = os.path.join(save_dir, "random_forest_height.joblib")
...
search.fit(X_train_s, y_train.ravel())
...
# ================= EVALUATION =================
y_pred = best_model.predict(X_test_s).reshape(-1, 1)
...
plt.plot([min(y_test)-0.0001, max(y_test)+0.0001],
         [min(y_test)-0.0001, max(y_test)+0.0001],
...
output_path_1 = os.path.join(save_dir, "Measured vs. Predicted Height.png")  # Specify the output file name and path
plt.savefig(output_path_1, dpi=300, bbox_inches='tight')  # Save the figure with high resolution
plt.show()
...
plt.tight_layout()
output_path_2 = os.path.join(save_dir, "Feature Importance.png")  # Specify the output file name and path
plt.savefig(output_path_2, dpi=300, bbox_inches='tight')  # Save the figure with high resolution
plt.show()

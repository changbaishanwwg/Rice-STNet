...
from commonfuction import HeightDataset
from simpleCNN import HeightCNN
import matplotlib.pyplot as plt
from sklearn.metrics import r2_score, root_mean_squared_error, mean_absolute_error
from commonfuction import create_result_subfolders
import numpy as np
...
# Define the model class (same structure as during training)
model = HeightCNN()
# Load the saved model weights
model.load_state_dict(torch.load("./result/Train/2/height_cnn_best_model.pth", weights_only=True))
...
with torch.no_grad():  # Disable gradient calculation
    for images, osavi_values in dataloader:
        images = images.to(device)
        osavi_values = osavi_values.to(device)

        # Forward pass
        predictions = model(images)
        predictions = predictions.cpu().numpy().flatten()  # Convert to NumPy
        measured = osavi_values.cpu().numpy()  # Convert to NumPy
